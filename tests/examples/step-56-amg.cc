...
namespace Step56
{
  ...
  struct SolverType
  {
    enum type {FGMRES_ILU, FGMRES_GMG, FGMRES_AMG, UMFPACK};
  };
  ...
  template <class PreconditionerAType, class PreconditionerSType, class BLOCKMAT=BlockSparseMatrix<double>, class MAT=SparseMatrix<double> >
  class BlockSchurPreconditioner : public Subscriptor
  {
  public:
    BlockSchurPreconditioner (const BLOCKMAT  &system_matrix,
                              const MAT &schur_complement_matrix,
  ...
  template <class PreconditionerAType, class PreconditionerSType, class BLOCKMAT, class MAT>
  BlockSchurPreconditioner<PreconditionerAType, PreconditionerSType, BLOCKMAT, MAT>::
  BlockSchurPreconditioner (const BLOCKMAT  &system_matrix,
                            const MAT &schur_complement_matrix,
                            const PreconditionerAType &preconditioner_A,
                            const PreconditionerSType &preconditioner_S,
...
// preconditioner as well as the solve itself.
  ...
  void StokesProblem<dim>::solve ()
  {
    ...
    if (solver_type == SolverType::FGMRES_AMG)
      {
        // make a copy of all our matrices. Inefficient!
        TrilinosWrappers::BlockSparseMatrix mat;
        mat.reinit(system_matrix);
        TrilinosWrappers::SparseMatrix mat_S;
        mat_S.reinit(pressure_mass_matrix);

        computing_timer.enter_subsection ("setup AMG");
        ...
        A_preconditioner.initialize(mat.block(0,0), data);

        TrilinosWrappers::PreconditionILU S_preconditioner;
        S_preconditioner.initialize(mat_S);

        const BlockSchurPreconditioner<TrilinosWrappers::PreconditionAMG, TrilinosWrappers::PreconditionILU>
    ...
    else if (solver_type == SolverType::FGMRES_ILU)
      {
...
// respective order.
  ...
  void StokesProblem<dim>::run ()
  {
    ...
    else if (solver_type == SolverType::FGMRES_AMG)
      std::cout << "Now running with AMG" << std::endl;
    else
  ...
  try
    {
      ...
      StokesProblem<dim> flow_problem(degree, SolverType::FGMRES_AMG);

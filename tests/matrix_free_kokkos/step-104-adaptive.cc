...
// Test step-104 with adaptive refinement.
...
namespace Step104
{
  ...
  StokesProblem<dim, degree_p, Number>::solve()
  {
    ...
    // MGLevelObject<MGTwoLevelTransferCopyToHost<dim, VectorType>>
    ...
    deallog << "GMG velocity block smoothers:" << std::endl;
    for (unsigned int level = min_level; level <= max_level; ++level)
      {
        VectorType vec;
        ...
        deallog << "    level: " << level << " n_dofs: " << vec.size()
                << ", eigenvalue spectrum: [ "

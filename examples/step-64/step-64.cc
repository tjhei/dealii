...
namespace stepXX
{
  ...
  template <int dim, int fe_degree, typename number>
  void JacobianOperator<dim, fe_degree, number>::evaluate_nonlinearity(
    const LinearAlgebra::distributed::Vector<number> &src)
  {
  ...
  template <int dim, int fe_degree, typename number>
  void JacobianOperator<dim, fe_degree, number>::vmult(
    LinearAlgebra::distributed::Vector<number> &      dst,
    const LinearAlgebra::distributed::Vector<number> &src) const
  {
  ...
  void GelfandProblem<dim>::setup_system()
  {
...

    {
      ...
      // Cannot activate also the boudnary flags for example for inhomogeneous
    ...
    // Now initialize the multilevel objects. Therefore we define a new
    // MatrixFree object for the level operators.
  ...
  template <int dim>
  void GelfandProblem<dim>::assemble_right_hand_side()
  {
  ...
  template <int dim>
  double GelfandProblem<dim>::compute_solution_norm()
  {
  ...
  template <int dim>
  void GelfandProblem<dim>::output_results() const
  {
    if (dof_handler.n_dofs() > 1e6 ||
        triangulation.n_global_active_cells() > 1e6)
      return;
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
      {
        ...
        for (unsigned int i = 0;
             ...
             ++i)
          {
            filenames.emplace_back("solution." + Utilities::to_string(i, 2) +
                                   ".vtu");
          }
  ...
  void GelfandProblem<dim>::run()
  {
    ...
    // Generate the triangulation and set the manifold objects to refine
    // correctly
    make_grid();
    ...
    // Setup the system, i.e., define the MatrixFree object and initialize the
    // system matrix as well as the residual operator. Initialize the vectors
    // and the multilevel object.
